  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="班级" prop="clazz">
        <el-select v-model="queryParams.clazz" placeholder="请选择班级" clearable>
          <el-option
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['lms:courseteacher:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['lms:courseteacher:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['lms:courseteacher:remove']"
        >删除</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="courseTeacherList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="班级" align="center" prop="clazz">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.clazz" :value="scope.row.clazz"/>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="班级" prop="clazz">
          <el-select v-model="form.clazz" placeholder="请选择班级" clearable>
            <el-option
              v-for="dict in dict.type.clazz"
              :key="dict.value"
              :label="dict.label"
...
<script>
import { listCourseTeacher, getCourseTeacher, delCourseTeacher, addCourseTeacher, updateCourseTeacher } from "@/api/lms/courseTeacher";

export default {
  name: "CourseTeacher",
  dicts: ['grade', 'clazz', 'course'],
  data() {
    return {
      // 遮罩层
      loading: true,
      ...
      rules: {
        grade: [
          { required: false, message: "年级不能为空", trigger: "change" }
        ],
        clazz: [
          { required: false, message: "班级不能为空", trigger: "change" }
        ],
        course: [
  ...
  methods: {
    ...
    resetQuery() {
      ...
      this.queryParams.clazz = null;
      this.queryParams.course = null;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateCourseTeacher(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addCourseTeacher(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除课程教师关联编号为"' + ids + '"的数据项？').then(function() {
